...
export class CommentsService {
  ...
  async create(createCommentDto: CreateCommentDto, userId: string): Promise<Comment> {
    try {
      ...
      const comment = this.commentRepository.create({
        ...
        activity_id: BigInt(createCommentDto.activity_id),
        parent_id: createCommentDto.parent_id ? BigInt(createCommentDto.parent_id) : null,
        likes: 0,
        status: 1, // 默认显示
      });
  ...
  async update(
    ...
    try {
      ...
      if (!isAdmin) {
        ...
        await this.commentRepository.update(comment.id, { content });
      } else {
        // 管理员可以更新状态等信息
   ...
   * @returns 
   */
  async remove(commentId: string, userId: string, isAdmin: boolean): Promise<{ message: string }> {
    try {
      // 查询要删除的评论是否存在
      const comment = await this.findOne(commentId);
   ...
   * @returns 
   */
  async likeComment(commentId: string): Promise<{ message: string; likes: number }> {
    try {
      // 查询评论是否存在
      const comment = await this.findOne(commentId);
      ...
      // 增加点赞数
      comment.likes += 1;
      await this.commentRepository.update(comment.id, { likes: comment.likes });

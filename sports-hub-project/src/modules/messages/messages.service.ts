...
export class MessagesService {
   ...
   * @returns 
   */
  async create(createMessageDto: CreateMessageDto, sender_id: string): Promise<Message> {
    try {
      // 生成业务消息ID
  ...
  async findOne(messageId: string, userId: string): Promise<Message> {
    try {
      ...
      // 如果是接收者查看，自动标记为已读
      if (message.receiver_id === userIdBigInt.toString() && message.read_status === 0) {
   ...
   * @returns 
   */
  async markAsRead(messageId: string, userId: string): Promise<{ message: string }> {
    try {
      // 查询消息是否存在
   ...
   * @returns 
   */
  async remove(messageId: string, userId: string, isAdmin: boolean): Promise<{ message: string }> {
    try {
      // 查询消息是否存在
      const message = await this.messageRepository.findOne({
        where: { message_id: messageId },
